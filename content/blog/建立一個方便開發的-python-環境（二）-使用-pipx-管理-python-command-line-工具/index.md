...
使用 `pipx run` 時 pipx 將會於 `$PIPX_HOME/.cache` 中開啟一個暫時的虛擬環境安裝該套件，直接執行該虛擬環境中 bin/ 下的執行檔，並且將要要執行的子指令或參數帶入，此暫時的虛擬環境會存在 14 天，如此在下次執行時會因為跳過了下載階段而執行比較快，若超過 14 天則會在下次執行 `pipx run` 時[將該虛擬環境刪除](https://github.com/pipxproject/pipx/blob/master/src/pipx/commands/run.py#L170)。
